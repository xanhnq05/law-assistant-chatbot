...
(() => {
  ...
  // ---------- CONFIG ----------
  // Đổi thành URL server FastAPI của bạn khi deploy.
  // Mặc định backend chạy ở http://localhost:8000
  const API_BASE_URL =
  ...
  async function authFetch(path, options = {}) {
    ...
    try {
      ...
      throw new Error(
        `Không kết nối được tới backend (${API_BASE_URL}). Hãy chắc chắn backend đang chạy ở cổng 8000.`
      );
    }
